...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const cateRes = await fetch(`${API_BASE}/categories?per_page=100`);
    ...
    async function fetchPosts(categoryId = "all") {
      ...
      if (categoryId !== "all") url += `&categories=${categoryId}`;
    ...
    async function search(post = 'all'){
      let url = `${API_BASE}/posts?embed&per_page=100`;
      if(categoryId !== 'all') url += `&categories=${categoryId}`
    }
    ...
    function renderPosts() {
      ...
      blogGrid.innerHTML = posts
        .map((p) => {
          ...
          const date = meta?.post_date || p?.date || "Now";
